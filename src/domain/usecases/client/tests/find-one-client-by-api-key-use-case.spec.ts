...
import FindOneClientByApiKeyUseCase from '../find-one-client-by-api-key.use-case';

describe('find one client by api key use case', () => {
  ...
  describe('when get one client by api key', () => {
    it('should return one client', async () => {
      ...
      const findOneByApiKeyMock =
        clientRepository.findOne.mockResolvedValueOnce(clientMock);

      const useCase = new FindOneClientByApiKeyUseCase(clientRepository);
      ...
      expect(findOneByApiKeyMock).toBeCalledTimes(1);
      expect(resultExpect).toEqual(output);
      expect.assertions(2);
    });

    it('should return not found error', async () => {
      const apiKey = '355242a5-4d0d-4199-bced-166ba023267d';

      const useCase = new FindOneClientByApiKeyUseCase(clientRepository);

      const exception = new DomainException(`Client not found: ${apiKey}`);
